...
class _CleaningInspectionContentState
  ...
  Future<void> _showAddScheduleDialog() async {
    ...
    if (uid != null) {
      final doc = await _firestore.collection('users').doc(uid).get();
      final floor = doc.data()?['assignedFloor'] as String?;
      if (floor != null && _floorOptions.contains(floor)) {
        profileFloor = floor;
      }
    }
    ...
    String selectedFloor = profileFloor ?? _floorOptions.first;
...
class _RoomInspectionHistoryContentState
  ...
  Future<void> _loadProfileFloor() async {
    ...
    if (floor != null && mounted) {
      // Parse "샬롬하우스 A동 3층" → building='A', floor=3
      final buildingMatch = RegExp(r'([A-Z])동').firstMatch(floor);
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              Wrap(
                                ...
                                runSpacing: 4,
                                children: [
                                  // kPersonalCheckStructure 순서대로 표시 (미흡 먼저, 양호 나중)
                                  ...() {
                                    final ordered = <MapEntry<String, dynamic>>[];
                                    for (final cat in kPersonalCheckStructure.entries) {
                                    ...
                                    // 미흡 먼저, 양호 나중
                                    final fails = ordered.where((e) => e.value is int ? (e.value as int) <= 2 : e.value != true);
                                    final passes = ordered.where((e) => e.value is int ? (e.value as int) >= 3 : e.value == true);
                                    return [...fails, ...passes].map((e) => _buildHistoryItemChip(e.key, e.value));
                                  }(),
                                ],
                              ),
                            ],
                      ...
                      Wrap(
                        ...
                        children: [
                          ...() {
                            ...
                            final ordered = <MapEntry<String, dynamic>>[];
                            for (final cat in kCommunalCheckStructure.entries) {
                              for (final sub in cat.value) {
                                final key = '${cat.key}_$sub';
